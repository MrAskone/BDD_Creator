...
void WManager::sendActionToCpp(QString nomAction, QString parametre/*=""*/)
{
    ...
    if ( nomAction == "addToList" )                         // Add to List Button
    {
        parametre = parametre.remove(0, 8);

        if (!directoryList.contains(parametre))
        {
            directoryList << parametre;
            updateQML_model("myListModel" , directoryList);
            setPropString ("Directory added");
        }
        else
        {
            setPropString("Can't add directory twice");
        }                                                   // End of Add to List
    ...
    else if( nomAction == "displayFileList")                // Display File List Button
    {
        if (directoryList.length() > 0)
        {
